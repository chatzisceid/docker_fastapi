...
def f(rays, ts, **kwargs):
    ...
    for i in range(0, B, args.chunk):
        ...
        if 'a_embedded' in kwargs:
            kwargs_['a_embedded'] = kwargs['a_embedded'][i:i+args.chunk]
        rendered_ray_chunks = \
            render_rays(models,
                        embeddings,
                        rays[i:i+args.chunk],
...
if __name__ == "__main__":
    ...
    #dataset = PhototourismDataset('/media/chatzise/E0E43345E4331CEA/Nepszinhaz_images/', split='test_train', img_downscale=2, use_cache=False)
    kwargs = {}
#### moving scene 
    ...
    if args.dataset_name == 'phototourism' :
        ...
        dataset.test_appearance_idx = 1 # 85572957_6053497857.jpg 1123
        N_frames = 120
        # dx = np.linspace(0, 0.03, N_frames)
        # dy = np.linspace(0, -0.1, N_frames)
        dz = np.linspace(0, 0.5, N_frames)
        radii = np.linspace (-10 ,10, N_frames)  # adjust as needed
        angle = np.linspace(0, 2*np.pi, N_frames)  # creates a range of angles from 0 to 2pi
        
        #dz = np.linspace(-0.5, 0.5, N_frames)  # adjust as needed if you want the camera to move up or down as it orbits
        #define poses
        dataset.poses_test = np.tile(dataset.poses_dict[1], (N_frames, 1, 1))
        for i in range(N_frames):
            radius = radii[i]
            dx = radius * np.cos(angle)  # x coordinate on the circle
            dy = radius * np.sin(angle)  # y coordinate on the circle
...
#### interpolation gif
    ...
    # dataset = PhototourismDataset('/media/chatzise/E0E43345E4331CEA/Nepszinhaz_images/',
    #         split='test_train',
    #         img_downscale=2, use_cache=False)

    # left_sample = dataset[15]
    # right_sample = dataset[23]
    ...
    #         imageio.imwrite(os.path.join(dir_name, f'{i:03d}l.png'), left_GT)
    #     elif i == 4:
    #         img_wh = tuple(results['img_wh'].numpy())
    #         right_GT = results['rgbs'].view(img_wh[1], img_wh[0], 3).cpu().numpy()
    #         imageio.imwrite(os.path.join(dir_name, f'{i:03d}.png'), right_GT)
    #     else:
