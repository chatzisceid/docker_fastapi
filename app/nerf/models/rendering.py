...
def sample_pdf(bins, weights, N_importance, det=False, eps=1e-5):
    ...
    denom[denom<eps] = 1 # denom equals 0 meansασε μολις εφαγα 
    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ=='coarse' and test_time:
            ...
            if model.encode_appearance:
                # a_embedded_ = a_embedded.expand(a_embedded.shape[0] + N_samples, -1)
    ...
    if N_importance > 0: # sample points for fine model
        ...
        if model.encode_appearance:
            if 'a_embedded' in kwargs:
                a_embedded: torch.Tensor = kwargs['a_embedded']
            else:
