...
if __name__ == "__main__":
    ...
    embedding_xyz = HashGrid(config, 3).to(device)
    embedding_dir = HashGrid(config, 3).to(device)
    embeddings = {'xyz': embedding_xyz, 'dir': embedding_dir}
    if args.encode_a:
        embedding_a = torch.nn.Embedding(args.N_vocab, args.N_a).cuda()
        embedding_a = nerf.embeddings['a']
        embeddings['a'] = embedding_a
    if args.encode_t:
        embedding_t = torch.nn.Embedding(args.N_vocab, args.N_tau).cuda()
        embedding_t = nerf.embeddings['t']
        embeddings['t'] = embedding_t

    nerf_coarse = Mlp('coarse',
                        in_channels_xyz=6*args.N_emb_xyz+6,
                        in_channels_dir=6*args.N_emb_dir+6).to(device)
    ...
    nerf_fine = Mlp('fine',
                     ...
                     beta_min=args.beta_min).to(device)

    nerf_coarse = nerf.nerf_coarse
    nerf_fine = nerf.nerf_fine
